...
Static Function ImprPV()
  ...
  Local cLogo     := GetSrvProfString("Startpath","") + "lgmid01.png"
	Local oFont8  	:= TFont():New("Arial",9,10,.T.,.F.,5,.T.,5,.T.,.F.)
	...
	Local cImpressora	:= "BEMATECH MP-4200 TH 01.00.00(S)"// LjGetStation("IMPFISC") //"BEMATECH MP4200 V01.00.00" // LjGetStation("IMPFISC")
	Local cPorta		:= "USB" // LjGetStation("PORTIF") //"USB" 
  Local sTexto := ""
  Local oPrint
Local nHdlECF
Alert("AQUI")
Alert(cImpressora)
ALert(cPorta)
//ALert(STWOpenCom(cImpressora, cPorta))
nHdlECF := INFAbrir( cImpressora,cPorta )
Alert(nHdlECF)
INFTexto("<n>TESTE 2<n>")
Alert("FOI")
cTxImp := "<n>TESTE 1<n>"
STWManagReportPrint(cTxImp,1)
Return
				...
				If nHdlECF == -1
						Alert("NFC-e: Não foi possível estabelecer comunicação com a Impressora:" + cImpressora)
						//nao ha necessidade de retornar erro quando houver erro de impressora
        else
        Alert("Abriu")
        INFTexto("<n>TESTE 1<n>")
        INFTexto("<e>" + PadC("CUPOM DE TROCA", Round(nMaxChar/2, 0)) + "</e>")
        	sTexto := sTexto + '<b>Data:</b> ' + DtoC(dDatabase) + ' <b>Hora: </b>' +Time() + Chr(13) + Chr(10)
	sTexto := sTexto + '<b>Vendedor:</b> ' + Alltrim(SC5->C5_VEND1)+' - ' +  Alltrim(SC5->C5_NUM) + Chr(13) + Chr(10)
	sTexto := sTexto + '<b>Caixa:</b> ' + Alltrim(SC5->C5_CLIENTE)+'<b> Operador: </b>' + Alltrim(SC5->C5_LOJACLI)+ Chr(13) + Chr(10)
...
//	oPrint:= FWMSPrinter():New("Amarelinha")
//oPrint := FWMSPrinter():New("Amarelinha",IMP_SPOOL,.F., "", .T., , @oPrint, "", , , , .T.)
cPrinter := "MP-4200 TH"
 lTReport := .F.
 lServer := .F.
  lPDFAsPNG := .F.
oPrint := FWMSPrinter():New("cFilePrintert" ,IMP_SPOOL/*IMP_PDF*/, lAdjustToLegacy, /*cPathInServer*/, lDisableSetup , lTReport, /*@oPrintSetup*/, cPrinter, lServer, lPDFAsPNG, /*lRaw*/, /*lViewPDF*/, /*nQtdCopy*/ )

// oPrint := FWMSPrinter():New("Amarelinha",IMP_SPOOL,lAdjustToLegacy, , lDisableSetup)
 oPrint:LPDFASPNG := .F.	
//	oPrint:= TMSPrinter():New("Amarelinha")
 //oPrint:SetPaperSize(0,4500,7500)
//oPrint:SetPaperSize(DMPAPER_B4)
