							...
							oFWriter:Write('<row>' + CRLF)
								While DA1->(!Eof()) .AND. ( DA1->DA1_FILIAL + DA1->DA1_CODTAB == DA0->DA0_FILIAL + DA0->DA0_CODTAB )
									IF SB1->(MSSeek(xFilial("SB1") + DA1->DA1_CODPRO))
										...
										oFWriter:Write('<DADOSDECLARANTE>'+AllTrim(FWSM0Util():GetSM0Data(cEmpAnt,cFilAnt,{"M0_FULNAME"})[1,2])+'</DADOSDECLARANTE>' + CRLF)
										oFWriter:Write('<CNPJ>'+AllTrim(FWSM0Util():GetSM0Data(cEmpAnt,cFilAnt,{"M0_CGC"})[1,2])+'</CNPJ>' + CRLF)
										oFWriter:Write('<IEST>'+AllTrim(FWSM0Util():GetSM0Data(cEmpAnt,cFilAnt,{"M0_INSC"})[1,2])+'</IEST>' + CRLF)
										oFWriter:Write('<XNOME>'+AllTrim(FWSM0Util():GetSM0Data(cEmpAnt,cFilAnt,{"M0_FULNAME"})[1,2])+'</XNOME>' + CRLF)
										...
										oFWriter:Write('<uCom>'+AllTrim(SB1->B1_UM)+'</uCom>' + CRLF)
										oFWriter:Write('<uTRIB>'+AllTrim(SB1->B1_SEGUM)+'</uTRIB>' + CRLF)
